...
@section('content')
      ...
      <div class="row">
            <div class="col-md-8">
            <form method="POST" action="{{ route('user.update', $user->id) }}" data="{role_id: 1}">
            @csrf

            <div class="form-group">
            <div class="row">
                <label for="name" class="col-md-3">{{ __('Name') }}</label>
               <div class="col-md-6"> <input id="name" class="block mt-1 w-full" type="text" name="name" value="{{ $user->name }}" :value="old('name')" required autofocus autocomplete="name" ></div>
</div>
<div class="row">
